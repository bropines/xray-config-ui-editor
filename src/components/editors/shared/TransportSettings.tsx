...
export const TransportSettings = ({ streamSettings = {}, onChange, isClient = false }: TransportProps) => {
    ...
    return (
        <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-800 space-y-4 mt-4 animate-in fade-in">
            ...
            {/* --- MAIN SELECTORS (Responsive Grid) --- */}
            ...
            {net === 'xhttp' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-800 pt-4 animate-in fade-in">
                    ...
                    <div>
                        ...
                        >
                            <option value="auto">Auto</option><option value="stream">Stream (Download)</option><option value="packet">Packet</option>
                        </select>
                    </div>
                    <div>
                        <label className="label-xs">Path</label>
                        <input className="input-base font-mono" placeholder="/"
                            value={streamSettings.xhttpSettings?.path || ""} onChange={e => update(['xhttpSettings', 'path'], e.target.value)} />
                    </div>
                    <div className="md:col-span-2">
                        <label className="label-xs">Host (Optional)</label>
                        <input className="input-base font-mono" placeholder="example.com"
                            value={streamSettings.xhttpSettings?.host || ""} onChange={e => update(['xhttpSettings', 'host'], e.target.value)} />
                    </div>
                    <div className="md:col-span-2">
                        <label className="label-xs">Extra Headers (JSON)</label>
                        <input className="input-base font-mono text-xs" placeholder='{"no-sse": "true"}'
                            value={JSON.stringify(streamSettings.xhttpSettings?.extra || {})}
                            onChange={e => { try { update(['xhttpSettings', 'extra'], JSON.parse(e.target.value)); } catch (err) {} }} />
                    </div>
                </div>
            )}
            ...
            {net === 'kcp' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-800 pt-4 animate-in fade-in">
                    ...
                    <div><label className="label-xs">Seed (Obfuscation)</label><input className="input-base font-mono" placeholder="password" value={streamSettings.kcpSettings?.seed || ""} onChange={e => update(['kcpSettings', 'seed'], e.target.value)} /></div>
                    <div><label className="label-xs">MTU</label><input type="number" className="input-base font-mono" placeholder="1350" value={streamSettings.kcpSettings?.mtu || 1350} onChange={e => update(['kcpSettings', 'mtu'], parseInt(e.target.value))} /></div>
                </div>
            )}
            ...
            {net === 'quic' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-800 pt-4 animate-in fade-in">
                    ...
                    <div><label className="label-xs">Key</label><input className="input-base font-mono" placeholder="key" value={streamSettings.quicSettings?.key || ""} onChange={e => update(['quicSettings', 'key'], e.target.value)} /></div>
                </div>
            )}
            ...
            {sec === 'reality' && (
                <div className="space-y-4 border-t border-slate-800 pt-4 animate-in fade-in">
                    <div className="flex justify-between items-center"><span className="text-xs font-bold text-purple-400">REALITY</span><Button variant="secondary" className="px-2 py-1 text-[10px]" onClick={handleGenKeys}>Gen Keys Pair</Button></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="col-span-full"><label className="label-xs">SNI</label><input className="input-base font-mono" placeholder="example.com" value={isClient ? (streamSettings.realitySettings?.serverName || "") : (streamSettings.realitySettings?.serverNames || []).join(', ')} onChange={e => { const val = e.target.value; update(['realitySettings', isClient ? 'serverName' : 'serverNames'], isClient ? val : val.split(',').map((s: string) => s.trim())); }} /></div>
                    </div>
                </div>
            )}
            ...
            {sec === 'tls' && (
                <div className="space-y-4 border-t border-slate-800 pt-4 animate-in fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="md:col-span-2"><label className="label-xs">Server Name (SNI)</label><input className="input-base font-mono" value={streamSettings.tlsSettings?.serverName || ""} onChange={e => update(['tlsSettings', 'serverName'], e.target.value)} /></div>
                        {isClient && <div className="flex items-center gap-2 mt-2"><input type="checkbox" className="w-4 h-4 rounded bg-slate-950 border-slate-700" checked={streamSettings.tlsSettings?.allowInsecure || false} onChange={e => update(['tlsSettings', 'allowInsecure'], e.target.checked)} /><label className="text-sm text-slate-300">Allow Insecure</label></div>}
                    </div>
                </div>
            )}
