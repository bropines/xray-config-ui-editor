        ...
        <div className={`space-y-4 animate-in fade-in ${isDownload ? 'bg-indigo-950/20 p-4 rounded-xl border border-indigo-500/30' : ''}`}>
            {isDownload && (
                <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs uppercase mb-2">
                    <Icon name="DownloadSimple" weight="bold" />
{t("Download-Specific XHTTP Settings")}
</div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Select 
                        ...
                        options={[
                            ...
                            { value: "packet-up", label: t("Packet-Up"), description: t("Highest compatibility") },
                            { value: "stream-up", label: t("Stream-Up"), description: t("Full duplex (Fast)") },
                            { value: "stream-one", label: t("Stream-One"), description: t("Single request") },
                        ]}
                    />
                <div>
            ...
            <div className="border-t border-slate-800 pt-4">
                ...
                >
                    ...
                    {t("Extra Settings (Headers, Padding, Performance)")}
                    </button>
...
{t("Packet-Up Mode Settings")}
                        ...
                        <div className="border-t border-slate-800/50 pt-2">
                            ...
                            >
                                ...
                                {t("XMUX Control (H2/H3 Multiplexing)")}
                                </button>
            ...
            {isClient && !isDownload && (
                <div className="border-t border-slate-800 pt-4">
                    ...
                    >
                        ...
                        {t("Download-Only Settings (Asymmetric Upload/Download)")}
                        </button>
                    ...
                    {showDownload && (
                        <div className="mt-4 space-y-4 animate-in slide-in-from-top-2">
                            <div className="bg-purple-900/10 border border-purple-500/20 p-4 rounded-xl space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="label-xs">{t("Download Address")}</label>
                                        <input className="input-base font-mono" placeholder={t("another-cdn.com or IP")}
                                            value={extra.downloadSettings?.address || ""}
                                            onChange={e => update(['extra', 'downloadSettings', 'address'], e.target.value)} />
                                    </div>
                                    <div>
                                        <label className="label-xs">{t("Download Port")}</label>
...
{t("Download Security")}
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <Select 
                                            ...
                                            options={[
                                                { value: "tls", label: "TLS" },
                                                { value: "reality", label: t("REALITY") },
                                            ]}
                                            className="w-full"
                                        />
                                        <input className="input-base text-xs font-mono" placeholder={t("SNI (optional)")}
                                ...
                                <div>
                                    <label className="label-xs">{t("Download Path (must match upload usually)")}</label>
