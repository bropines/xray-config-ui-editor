...
import { perLanguage, t, tn } from '../../i18n';

const uaPresets = perLanguage(() => [
    { value: "Happ/1.0.0", label: t("Happ (iOS / Android)") },
    { value: "v2rayNG/1.9.13", label: t("v2rayNG (Android)") },
    { value: "Shadowrocket/1982 CFNetwork/1408.0.4 Darwin/22.5.0", label: t("Shadowrocket (iOS)") },
    { value: "ClashMeta/v1.18.0", label: t("Clash.Meta / Mihomo") },
    { value: "sing-box/1.10.0", label: t("sing-box") },
    { value: "FoXray/1.4.2", label: t("FoXray") },
    { value: "NekoBox/1.3.1", label: t("NekoBox") },
    { value: "custom", label: t("Custom User-Agent...") },
]);
                ...
                {!parsedConfigs ? (
                    <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full space-y-4 py-4 min-h-0">
                        <div className="text-center space-y-2 shrink-0">
                            ...
                            <h3 className="text-xl font-black text-white italic tracking-tight">{t("Configuration Harvester")}</h3>
                            <p className="text-slate-500 text-xs max-w-md mx-auto leading-relaxed">
                                {t("Paste raw proxy links (VLESS, VMess, SS, Trojan, WG), JSON configs, or fetch directly from a subscription URL.")}
                                </p>
                        </div>
                        ...
                        <div className="bg-slate-900/60 p-3.5 rounded-2xl border border-slate-800/80 space-y-3 shrink-0">
                            <div className="flex flex-col md:flex-row gap-2">
                                ...
                                <Button variant="secondary" className="px-5 rounded-xl border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/10 shadow-lg" onClick={handleFetchSub} disabled={!subUrl.trim() || isFetching} icon="CloudArrowDown">
                                    {isFetching ? t("Fetching...") : t("Fetch Remote")}
                                </Button>
                            </div>
                            ...
                            <div className="space-y-3 pt-2 border-t border-slate-800/60">
                                <div className="flex flex-col sm:flex-row items-center gap-3">
                                    <label className="text-[11px] font-bold text-slate-400 shrink-0 flex items-center gap-1.5">
                                        <Icon name="DeviceMobile" className="text-indigo-400" />
{t("Emulated Client:")}
</label>
                                    ...
                                    {selectedUa === 'custom' && (
                                        <input
                                            ...
                                            placeholder={t("e.g. FoXray/1.4.2 (iPhone; iOS 17.5)")}
                                            value={customUa}
                                    ...
                                    >
                                        <Icon name="Fingerprint" /> {showHwidSettings ? t("Hide HWID") : t("Device HWID")}
                                    </button>
                                </div>
                                    ...
                                    <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-3 animate-in fade-in">
                                        <div className="flex items-center justify-between">
                                            <span className="text-[10px] font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                                                <Icon name="Fingerprint" className="text-purple-400 text-sm" />
{t("Remnawave Device & System Parameters")}
</span>
...
{t("New HWID")}
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-400">{t("Device HWID (x-hwid):")}</label>
                                            <div className="flex gap-2">
                                                <input
                                                    type="text"
                                                    ...
                                                    placeholder={t("UUID or 10-64 char alphanumeric string")}
                                                />
                                                <Button
                                                    variant="secondary"
                                                    size="sm"
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        toast.success(t("HWID copied to clipboard"));
                                                    }}
                                                    icon="Copy"
                                                >
                                                    {t("Copy")}
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1">
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400">{t("Device OS (x-device-os):")}</label>
                                                <input
                                                    type="text"
                                                    ...
                                                    placeholder={t("e.g. Windows, iOS, Android")}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400">{t("OS Version (x-ver-os):")}</label>
                                                <input
                                                    type="text"
                                                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-1 text-xs font-mono text-slate-200 outline-none focus:border-indigo-500"
                                                    value={verOs}
                                                    onChange={e => handleVerOsChange(e.target.value)}
                                                    placeholder={t("e.g. 10.0.26220 or 18.3")}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-[10px] font-bold text-slate-400">{t("Device Model (x-device-model):")}</label>
                                                <input
                                                    type="text"
                                                    className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-1 text-xs font-mono text-slate-200 outline-none focus:border-indigo-500"
                                                    value={deviceModel}
                                                    onChange={e => handleDeviceModelChange(e.target.value)}
                                                    placeholder={t("e.g. MS-7E06 / iPhone 16 Pro")}
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <p className="text-[9px] text-slate-500 pt-1 leading-relaxed">
                                            💡 <b>{t("Tip:")}</b>
{t("If your subscription has a strict 1-device limit, copy your existing HWID and OS from your active client (e.g. Throne / v2rayTun) above to fetch your real proxy nodes.")}
</p>
                                    </div>
                                )}
                            </div>
...
{t("Source Payload (JSON / Links / Base64)")}
</span>
...
{t("Beautify JSON")}
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            toast.success(t("Copied payload to clipboard"));
                                        }}
                                        disabled={!inputText.trim()}
                            ...
                            <div className="p-4 border-b border-slate-800/60 bg-slate-950/40 flex justify-between items-center">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs font-bold text-white mt-1">
                                        {tn(parsedConfigs.length, "{n} object found", "{n} objects found")}
                                    </span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Button
                                        ...
                                        title={t("Edit or view raw source input")}
                                    >
                                        <Icon name="ArrowLeft" />
{t("Source")}
</Button>
                            ...
                            <div className="bg-slate-900/60 border border-slate-800/60 p-5 rounded-3xl flex justify-between items-center shadow-xl backdrop-blur-xl">
                                ...
                                <div className="flex gap-2.5">
                                    <Button
                                        ...
                                        onClick={() => {
                                            navigator.clipboard.writeText(inputText);
                                            toast.success(t("Copied analyzed source payload to clipboard"));
                                        }}
                                        icon="Copy"
                                        title={t("Copy raw response/input that was analyzed")}
                                    >
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <h4 className="text-[11px] font-black text-emerald-400 uppercase tracking-[0.3em] flex items-center gap-2">
                                            <Icon name="ArrowCircleDown" weight="fill" />
{t("Inbound Harvester")}
</h4>
                                            ...
                                            <div key={i} className="bg-emerald-500/20 border border-emerald-500/40 p-4 rounded-2xl flex justify-between items-center group hover:border-emerald-500/60 hover:bg-emerald-500/20 transition-all shadow-lg text-left">
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2 mt-1">
                                                        ...
                                                        <span className="text-[10px] text-slate-500 font-mono font-bold">{t("PORT")} {ib.port}</span>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
                                                                                                    <button onClick={() => openSectionJson('inbound', `JSON: ${ib.tag}`, ib)} title={t("Edit JSON")} className="p-2 rounded-md bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors"><Icon name="Code" weight="bold" /></button>
                                                                                                    <button onClick={() => openInboundEditor(ib)} title={t("Open GUI Editor")} className="p-2 rounded-md bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors"><Icon name="PencilSimple" weight="bold" /></button>
                                                                                                    <button onClick={() => importInbound(ib)} title={t("Add to Config")} className="p-2 rounded-md bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all"><Icon name="Plus" weight="bold" /></button>
                                                                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                                        {(selectedConfig?.outbounds || []).filter((o: any) => !['freedom', 'blackhole', 'dns'].includes(o.protocol)).map((ob: any, i: number) => (
                                            <div key={i} className="bg-indigo-500/10 border border-indigo-500/30 p-4 rounded-2xl flex justify-between items-center group hover:border-indigo-500/60 hover:bg-indigo-500/20 transition-all shadow-lg text-left">
                                                ...
                                                <div className="flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
                                                                                                    <button onClick={() => openSectionJson('outbound', `JSON: ${ob.tag}`, ob)} title={t("Edit JSON")} className="p-2 rounded-md bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors"><Icon name="Code" weight="bold" /></button>
                                                                                                    <button onClick={() => openOutboundEditor(ob)} title={t("Open GUI Editor")} className="p-2 rounded-md bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-colors"><Icon name="PencilSimple" weight="bold" /></button>
                                                                                                    <button onClick={() => importOutbound(ob)} title={t("Add to Config")} className="p-2 rounded-md bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500 hover:text-white transition-all"><Icon name="Plus" weight="bold" /></button>
                                                                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <h4 className="text-[11px] font-black text-purple-400 uppercase tracking-[0.3em] flex items-center gap-2">
                                                <Icon name="ArrowsSplit" weight="fill" />
{t("Rules")}
</h4>
