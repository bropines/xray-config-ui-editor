...
export const HostsModal = ({ onClose, initialHostUuid, onOpenTemplates }: {
    ...
    return (
        <Modal
            ...
            saveText={h.saving ? 'Saving…' : (h.isNew ? 'Create host' : 'Save host')}
            ...
            extraButtons={draft && !h.isNew ? (
                h.confirmDelete ? (
                    ...
                    <Button variant="ghost" icon="Trash" onClick={() => h.setConfirmDelete(true)}>Delete host</Button>
                )
            ) : null}
        >
                    ...
                    <div className="p-3 border-b border-slate-800 space-y-2.5 bg-slate-900/50 shrink-0">
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Search remark, address, tag…"
                                value={h.search}
                                onChange={e => h.setSearch(e.target.value)}
                            />
                        </div>
                        <Switch checked={h.showHidden} onChange={h.setShowHidden} label="Show hidden hosts" />
                        ...
                        <div className="text-[10px] text-slate-500">{h.rows.length} of {h.hostCount} host(s)</div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scroll p-2">
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    <span className="flex items-center gap-1.5 flex-wrap">
                                        ...
                                        {row.isDisabled && <Badge variant="warning" size="sm">off</Badge>}
                                        {row.isHidden && <Badge variant="info" size="sm">hidden</Badge>}
                                        {row.hasTemplate && <Badge variant="primary" size="sm">template</Badge>}
                                    </span>
                        ...
                        {h.rows.length === 0 && h.connected && (
                            <div className="text-center text-slate-600 py-8 italic text-[11px] px-3">
                                {h.hostCount === 0 ? 'No hosts loaded — press Refresh.' : 'No host matches that search.'}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className={`flex-1 flex-col min-h-0 min-w-0 bg-slate-900/50 ${draft ? 'flex' : 'hidden md:flex'}`}>
                    {!draft ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-600 p-6 text-center">
                            ...
                            <p className="text-sm">Pick a host to edit it</p>
                            <p className="text-[11px] text-slate-700 mt-2 max-w-sm">
                                Everything a subscriber needs to connect lives here: address and port,
                                the transport details, the inbound it serves, and the template it
                                renders. No client UUID involved — that only matters when turning a
                                host into a node in the balancer builder.
                            </p>
                        </div>
                    ) : (
                        <div className="flex-1 min-h-0 overflow-y-auto custom-scroll p-4 md:p-5 flex flex-col gap-3">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <Badge variant={h.isNew ? 'warning' : 'primary'} size="sm" icon="Broadcast">
                                    {h.isNew ? 'New host' : 'Editing host'}
                                </Badge>
                                {!h.isNew && h.isDirty && <Badge variant="warning" size="sm" icon="PencilSimple">Unsaved changes</Badge>}
                                {!h.isNew && !h.isDirty && <Badge variant="success" size="sm" icon="Check">Saved</Badge>}
                            </div>
                            ...
                            <Section title="Identity">
                                <Input
                                    ...
                                    hint="What the subscriber sees in their client"
                                />
                            ...
                            <Section title="Where it points">
                                <Select
                                    ...
                                    options={[
                                        { value: '', label: h.inboundOptions.length ? 'Pick an inbound…' : 'No inbounds loaded — press Refresh' },
                                        ...h.inboundOptions.map(i => ({ value: i.uuid, label: i.label })),
                                    ]}
                                    hint="The config-profile inbound this host serves"
                                />
                                <Select
                                    label="Xray JSON template"
                                    value={draft.xrayJsonTemplateUuid}
                                    onChange={v => h.update({ xrayJsonTemplateUuid: v })}
                                    options={[
                                        { value: '', label: 'None — plain host' },
                                        ...h.templateOptions.map((t: any) => ({ value: t.uuid, label: t.name })),
                                    ]}
                                    hint="Attach one and subscribers of this host get that whole config, balancer included"
                                />
                                {onOpenTemplates && (
                            ...
                            <Section title="Transport">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <Input
                                        ...
                                        placeholder="empty = use the inbound's"
                                    />
                                    <Input
                                        label="Host header"
                                        value={draft.host}
                                ...
                                <Input
                                    ...
                                    hint="Used by xhttp, ws and httpupgrade; gRPC reads it as the service name"
                                />
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    <Select
                                        label="Fingerprint"
                                        ...
                                        options={[
                                            { value: '', label: 'Leave to the client' },
                            ...
                            <Section title="Visibility">
                                <Switch
                                    ...
                                    label="Hidden from subscribers"
                                />
                                <p className="text-[10px] text-slate-500 -mt-2 ml-[52px]">
                                    Hidden hosts are the pool a template injects — they never appear in a
                                ...
                                />
                                <p className="text-[10px] text-slate-500 -mt-2 ml-[52px]">
                                    Kept in the panel but served to nobody.
                                </p>
                            </Section>

                            {h.missing.length > 0 && (
                                <p className="text-[10px] text-amber-300/80">Still needed: {h.missing.join(', ')}</p>
                            )}
